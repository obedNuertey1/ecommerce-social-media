    ...
    useEffect(()=>{
        const pageLoaded = ()=>{
            if(localStorage.getItem("passkey")){
                ...
                localStorage.setItem("passkey_logs", JSON.stringify([...JSON.parse(localStorage.getItem("passkey_logs")), {"passkeyName": localStorage.getItem("passkeyName"), "privileges": JSON.parse(localStorage.getItem("privileges")), "accessiblePages": JSON.parse(["productspage", "analyticspage", "infopage"]), "activity": "Homepage", "activityDetails": "User entered Products Page", "date": new Date().toISOString()}]));
